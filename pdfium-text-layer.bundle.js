var PdfiumTextLayer = (() => {
  ...
  // pdfium-text-layer.js
  var pdfium_text_layer_exports = {};
  __export(pdfium_text_layer_exports, {
    buildTextLayer: () => buildTextLayer,
    destroy: () => destroy,
  ...
  async function init() {
    if (library) return;
    const wasmUrl = typeof chrome !== "undefined" && chrome.runtime ? chrome.runtime.getURL("vendor/pdfium.wasm") : "vendor/pdfium.wasm";
    library = await PDFiumLibrary2.init({ wasmUrl });
  }
  ...
  function groupItemsByLine(items) {
    ...
    for (const item of items) {
      if (item.str === void 0 || !item.transform) continue;
  ...
  return __toCommonJS(pdfium_text_layer_exports);
})();
