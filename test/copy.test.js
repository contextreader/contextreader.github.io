...
console.log('the site tells the truth about languages, however it presents them:');
{
    ...
    // Microsoft Clarity was tried 21–22 Sep behind a consent card and removed
    // (Ian). It is NOT cookieless — its loader sets Microsoft's MUID on
    // clarity.ms and bing.com — so it is not on this list; the check below the
    // page loop keeps it gone.
    ...
    ok('Microsoft Clarity is gone from every page', all.every(([, p]) => !/clarity\.ms|src="clarity\.js"/i.test(p)),
        all.filter(([, p]) => /clarity\.ms|src="clarity\.js"/i.test(p)).map(([f]) => f));
    ok('and its loader is gone', !fs.existsSync(path.join(ROOT, 'docs/clarity.js')));
