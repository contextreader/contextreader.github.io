...
Two honest caveats. Typefaces ship inside the extension for every language except Chinese,
Japanese and Korean — those three still fetch one file from Google Fonts when you open a
bubble, with no referrer, so Google sees your IP address but not the page you're on. And
Google sees your lookups, because it is the one answering them. Full detail in [PRIVACY_POLICY.md](PRIVACY_POLICY.md).
