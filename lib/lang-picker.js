...
(function (root, factory) {
    ...
    // One label everywhere. "English — English" says nothing twice.
    function label(l) {
    ...
    function options(langs) {
        return langs.map((l) => `<option value="${esc(l.code)}">${esc(label(l))}</option>`).join('');
    }

    // select, input — required elements. status — optional line for the match
    // count. languages — listLangs() output. current — selected code.
    function attach({ select, input, status, languages, current }) {
        let cur = current;
        let listing = false;
        ...
        function list(query) {
            ...
            // size 1 renders as a dropdown again, so a single match still gets two rows
            select.setAttribute('size', String(Math.max(2, Math.min(MAX_ROWS, matches.length))));
        ...
        return {
            // The popup and Settings can both change the language; keep up.
            setCurrent(code) { cur = code; if (!listing) select.value = code; },
        };
    }
