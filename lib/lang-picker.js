...
(function (root, factory) {
    ...
    function attach({ select, input, status, note, languages, current }) {
        ...
        select.addEventListener('change', () => {
            nativeChangeAt = Date.now();
            cur = select.value;
            if (listing) { input.value = ''; collapse(); }
            else showNote();
        });

        // ...except when the row clicked is the one already highlighted: a select
        // fires `change` only when its value CHANGES, and the first match is
        // highlighted the moment you type. So the most natural action there is —
        // search, click the single result — did nothing, and only Enter worked
        // (Ian, 23 Sep). Commit on the click itself, unless a change just did it.
        select.addEventListener('click', () => {
            if (!listing || select.selectedIndex < 0) return;
            if (Date.now() - nativeChangeAt < 50) return;
            choose(select.value);
        });
